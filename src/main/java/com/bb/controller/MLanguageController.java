...
import com.bb.primary.model.MLanguage;
import com.bb.primary.repository.MLanguageRepository;

@RestController
@RequestMapping("admin/language")
public class MLanguageController {
	@Autowired
	MLanguageRepository mCountryRepository;
	@CrossOrigin
	@GetMapping("/list")
	...
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestBody MLanguage country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		MLanguage country2 = mCountryRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@PathVariable(value = "id") Integer id) {
		...
		MLanguage country = mCountryRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestBody SearchObject<MLanguage> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<MLanguage> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage="+searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<MLanguage> countryList = mCountryRepository.findAll(example, pageable);
