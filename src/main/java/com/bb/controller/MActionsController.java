...
import com.bb.primary.model.MAction;
import com.bb.primary.repository.MActionRepository;

@RestController
@RequestMapping("admin/mAction")
public class MActionsController {
	@Autowired
	MActionRepository mActionRepository;
	@CrossOrigin
	@GetMapping("/list")
	public Response viewHomePage(@RequestHeader(value = "projectId", required = false) String projectId,@RequestHeader(value = "siteId", required = false) String siteId,Model model) {
		...
		try {
			res.setPayload(mActionRepository.findAll());
	...
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestHeader(value = "projectId", required = false) String projectId,@RequestHeader(value = "siteId", required = false) String siteId,@RequestBody MAction country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		MAction country2 = mActionRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@RequestHeader(value = "projectId", required = false) String projectId,@RequestHeader(value = "siteId", required = false) String siteId,@PathVariable(value = "id") Long id) {
		...
		MAction country = mActionRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestHeader(value = "projectId", required = false) String projectId,@RequestHeader(value = "siteId", required = false) String siteId,@RequestBody SearchObject<MAction> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<MAction> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage=" + searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<MAction> countryList = mActionRepository.findAll(example, pageable);
