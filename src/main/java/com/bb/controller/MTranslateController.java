...
import com.bb.primary.model.MTranslate;
import com.bb.primary.repository.MTranslateRepository;

@RestController
@RequestMapping("admin/translate")
public class MTranslateController {
	@Autowired
	MTranslateRepository mCountryRepository;
	@CrossOrigin
	@GetMapping("/list")
	...
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestBody MTranslate country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		MTranslate country2 = mCountryRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@PathVariable(value = "id") Long id) {
		...
		MTranslate country = mCountryRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestBody SearchObject<MTranslate> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<MTranslate> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage="+searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<MTranslate> countryList = mCountryRepository.findAll(example, pageable);
