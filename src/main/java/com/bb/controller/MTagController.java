...
import com.bb.primary.model.MTag;
import com.bb.primary.repository.MTagRepository;

@RestController
@RequestMapping("admin/mtag")
public class MTagController {
	...
	public Response viewHomePage(Model model) {
		...
		res.setPayload(mTagRepository.findAll());
		res.setMessage("sucessful");
		return res;
	}
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestBody MTag country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		MTag country2 = mTagRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@PathVariable(value = "id") Long id) {
		...
		MTag country = mTagRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestBody SearchObject<MTag> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<MTag> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage="+searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<MTag> countryList = mTagRepository.findAll(example, pageable);
