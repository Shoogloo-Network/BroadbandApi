...
import com.bb.primary.model.AdminRoleUserRel;
import com.bb.primary.repository.AdminRoleUserRelRepository;

@RestController
@RequestMapping("admin/adminRoleUserRel")
public class AdminRoleUserController {
	@Autowired
	AdminRoleUserRelRepository mCountryRepository;
	@CrossOrigin
	@GetMapping("/list")
	...
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestBody AdminRoleUserRel country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		AdminRoleUserRel country2 = mCountryRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@PathVariable(value = "id") Integer id) {
		...
		AdminRoleUserRel country = mCountryRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestBody SearchObject<AdminRoleUserRel> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<AdminRoleUserRel> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage="+searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<AdminRoleUserRel> countryList = mCountryRepository.findAll(example, pageable);
