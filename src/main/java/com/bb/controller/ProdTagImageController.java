...
import com.bb.primary.model.ProdTagImageRel;
import com.bb.primary.repository.ProdTagImageRelRepository;

@RestController
@RequestMapping("admin/prodTagImage")
public class ProdTagImageController {
	@Autowired
	ProdTagImageRelRepository mCountryRepository;
	@CrossOrigin
	@GetMapping("/list")
	...
	@CrossOrigin
	@PostMapping("/save")
	public Response saveEmployee(@RequestBody ProdTagImageRel country) {
		// public Response search(@ModelAttribute("country") MCountry country ) {
		System.out.println("country=" + country);
		ProdTagImageRel country2 = mCountryRepository.save(country);
		Response res = new Response();
	...
	public Response updateForm(@PathVariable(value = "id") Long id) {
		...
		ProdTagImageRel country = mCountryRepository.getReferenceById(id);
	...
	@CrossOrigin
	@PostMapping("search")
	public Response search(@RequestBody SearchObject<ProdTagImageRel> searchObject) {

		System.out.println("countr=" + searchObject);
		Example<ProdTagImageRel> example = Example.of(searchObject.getSearch());
		System.out.println("searchObject.getPage="+searchObject.getPage());
		Pageable pageable = searchObject.getPage().createAndGetPageable();
		Page<ProdTagImageRel> countryList = mCountryRepository.findAll(example, pageable);
